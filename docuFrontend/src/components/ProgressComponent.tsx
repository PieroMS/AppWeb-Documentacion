...
const ProgressComponent = () => {
    const [progressData, setProgressData] = useState([]); // Estado para almacenar data de progresos (lista)
    const [loading, setLoading] = useState(true); // Estado para gestionar la carga
    const [showModal, setShowModal] = useState(false); // Controla la visibilidad del modal
    const [progressToDelete, setProgressToDelete] = useState(null); // Controla qué progreso eliminar
    ...
    const listDataProgress = async () => {
        try {
            ...
            const response = await axios.get(`${config.apiUrl}/admin/progress/listProgress`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setProgressData(response.data.data); // Guardar los datos obtenidos en el estado
    ...
    const handleDeleteClick = (progress) => {
        setProgressToDelete(progress); // Establecer el progreso a eliminar
    ...
    const confirmDelete = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${config.apiUrl}/admin/progress/deleteProgress/${progressToDelete.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setProgressData(progressData.filter(item => item.id !== progressToDelete.id)); // Actualizar la lista de progresos
    ...
    const cancelDelete = () => {
        ...
        setProgressToDelete(null); // Resetear el progreso a eliminar
    };
    ...
    return (
        <>
                ...
                <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white dark:bg-[#37496B] p-6 rounded-xl shadow-2xl border border-gray-300 dark:border-gray-700 max-w-sm w-full">
                        <h2 className="text-lg dark:text-[#fff] font-semibold dark:text-gray-200 mb-4">
                            ¿Seguro que quieres eliminar "{progressToDelete?.name}"?
                        </h2>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h2 className="text-title-md2 font-semibold text-black dark:text-white">Progresos</h2>
                <nav>
                    <ol className="flex items-center gap-2">
                        <li>
                            <Link to="/inicio" className='dark:text-gray-200'>Inicio /</Link>
                        </li>
                        <li className="text-[#1d4ed8] dark:text-[#93c5fd]">Progresos</li>
                    </ol>
                </nav>
            </div>

            <NavLink
                to="/progresos/agregar"
                ...
                <table className="w-full table-auto dark:bg-boxdark">
                    <thead>
                        <tr className="bg-[#37496B] text-left">
                            ...
                            <th className="min-w-[150px] py-4 font-medium text-white dark:text-white">Color</th>
                        ...
                        {progressData.map((progress, index) => (
                            <tr key={index}>
                                <td className="border-b border-[#eee] py-5 pl-9 dark:border-strokedark pl-11 bg-[#eceaea] dark:bg-boxdark">
                                    <h5 className="font-medium text-black dark:text-white">{progress.name}</h5>
                                </td>
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark bg-[#eceaea] dark:bg-boxdark">
                                    <p className="text-black dark:text-white">{progress.color}</p>
                                </td>
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark bg-[#eceaea] dark:bg-boxdark">
                                    <div className="flex space-x-4">
                                        <Link to={`/progresos/editar/${progress.id}`}>
                                        ...
                                        <img
                                            ...
                                            onClick={() => handleDeleteClick(progress)} // Abre el modal
                                        />
                                    </div>
                                </td>
                            </tr>
