...
const RequestsTypeEditComponent = () => {
    const { id } = useParams(); // Obtiene el id de los parámetros de la URL
    const [requestaName, setRequestName] = useState(''); // Estado para el nombre
    ...
    useEffect(() => {
        const fetchRequest = async () => {
            try {
                ...
                const response = await axios.get(`${config.apiUrl}/admin/RequestsType/listRequestsTypeById/${id}`, {
                    headers: {
                ...
                console.error('Error al obtener el request:', error);
            }
        };
    ...
    const updateRequest = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.put(
                `${config.apiUrl}/admin/RequestsType/updateRequestsType/${id}`,
                {
                    name: requestaName
                },
                {
            ...
            navigate('/tipo-requests', { state: { successMessage: 'Dato actualizado correctamente.' } });
        } catch (error) {
                ...
                <nav>
                    <ol className="flex items-center gap-2">
                        <li>
                            ...
                            <Link to="/tipo-requests" className='dark:text-gray-200'>Request /</Link>
                        </li>
...
export default RequestsTypeEditComponent;