...
const RolesComponent = () => {
    const [rolesData, setRolesData] = useState([]); // Estado para almacenar data de Roless (lista)
    const [loading, setLoading] = useState(true); // Estado para gestionar la carga
    const [showModal, setShowModal] = useState(false); // Controla la visibilidad del modal
    const [rolesToDelete, setRolesToDelete] = useState(null); // Controla qué progreso eliminar
    ...
    const listDataRoles = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${config.apiUrl}/admin/roles/listRoles`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setRolesData(response.data.data); // Guardar los datos obtenidos en el estado
    ...
    useEffect(() => {
        listDataRoles(); // Llamar a la función al montar el componente
    }, []);
    
    const handleDeleteClick = (roles) => {
        setRolesToDelete(roles); // Establecer el progreso a eliminar
    ...
    const confirmDelete = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${config.apiUrl}/admin/roles/deleteRoles/${rolesToDelete.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setRolesData(rolesData.filter(item => item.id !== rolesToDelete.id)); // Actualizar la lista de progresos
    ...
    const cancelDelete = () => {
        ...
        setRolesToDelete(null); // Resetear el progreso a eliminar
    };
    ...
    return (
        <>
                ...
                <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
                    <div className="bg-[#e5e7eb] dark:bg-[#37496B] p-6 rounded-xl shadow-2xl border border-gray-300 dark:border-[#c2c5ca] max-w-sm w-full">
                        <h2 className="text-lg dark:text-[#fff] font-semibold mb-1">
                            ¿Seguro que quieres eliminar "{rolesToDelete?.name}"?
                        </h2>
                        <span className="text-lg dark:text-[#fff] mb-4">
                            Esta acción eliminará a los usuarios con este rol.
                        </span>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h2 className="text-title-md2 font-semibold text-black dark:text-white">Roles</h2>
                <nav>
                    <ol className="flex items-center gap-2">
                        <li>
                            <Link to="/inicio" className='dark:text-gray-200'>Inicio /</Link>
                        </li>
                        <li className="text-[#1d4ed8] dark:text-[#93c5fd]">Roles</li>
                    </ol>
                </nav>
            </div>
                ...
                <table className="w-full table-auto dark:bg-boxdark">
                    <thead>
                        <tr className="bg-[#37496B] text-left">
                            ...
                            <th className="min-w-[150px] py-4 font-medium text-white dark:text-white">Descripción</th>
                        ...
                        {rolesData.map((roles, index) => (
                            <tr key={index}>
                                <td className="border-b border-[#eee] py-5 pl-9 dark:border-strokedark pl-11 bg-[#eceaea] dark:bg-boxdark">
                                    <h5 className="font-medium text-black dark:text-white">{roles.name}</h5>
                                </td>
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark bg-[#eceaea] dark:bg-boxdark">
                                    <p className="text-black dark:text-white">{roles.description}</p>
                                </td>
                                <td className="border-b border-[#eee] py-5 dark:border-strokedark bg-[#eceaea] dark:bg-boxdark">
                                    <div className="flex space-x-4">
                                        <Link to={`/roles/editar/${roles.id}`}>
                                        ...
                                        <img
                                            ...
                                            onClick={() => handleDeleteClick(roles)} // Abre el modal
                                        />
                                    </div>
                                </td>
                            </tr>
