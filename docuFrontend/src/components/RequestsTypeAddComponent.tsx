...
const RequestsTypeAddComponent = () => {
    const [requestaName, setRequestName] = useState(''); // Estado para el nombre
    const navigate = useNavigate();
    const [validationErrors, setValidationErrors] = useState({});

    const addARequest = async (e) => {
        e.preventDefault();

        try {
            const token = localStorage.getItem('token');
            await axios.post(
                `${config.apiUrl}/admin/RequestsType/createRequestsType`, 
                {
                    name: requestaName
                },
                {
            ...
            navigate('/tipo-requests', { state: { successMessage: 'Dato agregado correctamente.' } });
        } catch (error) {
                ...
                <nav>
                    <ol className="flex items-center gap-2">
                        <li>
                            ...
                            <Link to="/tipo-requests" className='dark:text-gray-200'>Requests /</Link>
                        </li>
...
export default RequestsTypeAddComponent;
